...
if command -v lcov >/dev/null 2>&1; then
  echo "[coverage] Generating HTML report with lcov..."
  
  # Capture coverage data
  lcov --capture --directory "$BUILD_DIR" --output-file coverage.info --rc lcov_branch_coverage=1 2>/dev/null || true
  
  # Remove coverage for test files and system headers
  lcov --remove coverage.info "${TEST_DIR}/*" '/usr/*' --output-file coverage_filtered.info --rc lcov_branch_coverage=1 2>/dev/null || true
  
  # Remove graph.c from coverage report (consistent with gcov exclusion)
  lcov --remove coverage_filtered.info "*graph.c" --output-file coverage_filtered.info --rc lcov_branch_coverage=1 2>/dev/null || true
  
  # Generate HTML report
  genhtml coverage_filtered.info --output-directory "$HTML_DIR" --branch-coverage --rc lcov_branch_coverage=1 2>/dev/null || true
  
  if [[ -d "$HTML_DIR" ]]; then
    echo "[coverage] HTML report generated in $HTML_DIR/index.html"
  fi
  
  # Clean up intermediate files
  rm -f coverage.info coverage_filtered.info
else
  echo "[coverage] lcov not found - skipping HTML report generation"
  echo "           Install with: apt-get install lcov"
fi
